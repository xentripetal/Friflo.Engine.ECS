...
/// <summary>
///     Used to filter structural changes made to an entity like added / removed components / tags using
///     <see cref="HasEvent" />.<br />
...
public sealed class EventFilter
{
    ...
    /// <summary>
    ///     Enable filtering remove component events of the given <see cref="IComponent" /> type
    ///     <typeparamref name="T" />.
    /// </summary>
    ...
    private bool ContainsComponentEvent(int entityId)
    {
        ...
        for (var n = 0; n < filters.count; n++)
        {
            ...
            var hasComponent = componentTypes.bitSet.Has(filter.index);
            var addedFilter = filter.action == EntityEventAction.Added;
    ...
    private bool ContainsTagEvent(int entityId)
    {
        ...
        for (var n = 0; n < filters.count; n++)
        {
            ...
            var hasTag = tags.bitSet.Has(filter.index);
            var addedFilter = filter.action == EntityEventAction.Added;
    ...
    [Browse(Never)]
    private readonly EntityEvents componentEvents;
    [Browse(Never)]
    private readonly EntityEvents tagEvents;

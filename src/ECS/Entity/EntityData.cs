...
public readonly ref struct EntityData
{
    ...
    /// <exception cref="NullReferenceException"> if the entity is deleted.</exception>
    public EntityComponents Components => new (new Entity(Archetype.entityStore, Id));
    ...
    /// <exception cref="NullReferenceException"> if the entity is deleted.</exception>
    public bool Has<T>() where T : struct, IComponent => heapMap[StructInfo<T>.Index] != null;

    /// <summary>Return the component of the given type as a reference.</summary>
    /// <exception cref="NullReferenceException">
    ///     if the entity is deleted or has no component of Type
    ///     <typeparamref name="T" />
    /// </exception>
    /// <remarks>Executes in O(1)</remarks>
    public ref T Get<T>() where T : struct, IComponent => ref ((StructHeap<T>)heapMap[StructInfo<T>.Index]).components[compIndex];
    ...
    internal EntityData(int id) => Id = id;

    ...
    where T : struct, IComponent
{
    ...
    [Browse(Never)]
    internal readonly int target; //  4
    [Browse(Never)]
    internal readonly EntityRelations relations; //  8
    ...
    public EntityLinkEnumerator<T> GetEnumerator() => new (this);
    ...
    where T : struct, IComponent
{
    ...
    public EntityLink<T> Current => new (target, entities[index], relations);
...
sealed class EntityLinksDebugView<T> where T : struct, IComponent
{
    private readonly EntityLinks<T> entityLinks;

    internal EntityLinksDebugView(EntityLinks<T> entityLinks) => this.entityLinks = entityLinks;

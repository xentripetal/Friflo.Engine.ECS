...
partial struct StoreExtension
{
    // --------------------------------- script methods ---------------------------------
    internal static Script[] GetScripts(Entity entity, int scriptIndex) => entity.store.extension.entityScripts[scriptIndex].scripts;
    ...
    internal void AppendScript(Entity entity, Script script)
    {
        ...
        if (!entity.store.extension.scriptMap.TryGetValue(entity.Id, out var scriptIndex))
        {
            ...
            entityScripts[lastIndex] = new EntityScripts(entity.Id, new Script[]
            {
                script
            });
        }
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <see cref="EntityNode.archetype" />
    ///         </item>
    ///         <item>
    ///             <see cref="EntityNode.compIndex" />
    ///         </item>
    ///         <item>
    ///             <see cref="RawEntityNode.archIndex" />
    ///         </item>
    ///     </list>
    /// </remarks>
    internal Script AddScript(Entity entity, Script script, ScriptType scriptType)
    {
        ...
        script.entity = entity;
        if (!scriptMap.TryGetValue(entity.Id, out var scriptIndex))
        {
            // case: entity has not scripts => add new Scripts entry

...
abstract class SignalHandler
{
    ...
    internal static int NewEventIndex(Type type) => _nextEventIndex++;
}

sealed class SignalHandler<TEvent> : SignalHandler where TEvent : struct
{
    ...
    internal SignalHandler() => entityEvents = new Dictionary<int, Action<Signal<TEvent>>>();

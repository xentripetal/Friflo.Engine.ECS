...
public readonly struct ChildEntities : IEnumerable<Entity>
{
    ...
    public Entity this[int index] => new (store, node.childIds.GetAt(index, store.extension.childHeap));
    ...
    [Browse(Never)]
    internal readonly TreeNode node; //  8
    [Browse(Never)]
...
public struct ChildEnumerator : IEnumerator<Entity>
{
    ...
    // --- IEnumerator<>
    public readonly Entity Current => new (store, childIds.GetAt(index - 1, heap));
...
class ChildEntitiesDebugView
{
    [Browse(Never)]
    private ChildEntities childEntities;

    internal ChildEntitiesDebugView(ChildEntities childEntities) => this.childEntities = childEntities;

    ...
    where T3 : struct, IComponent
{
    ...
    internal QueryChunks(ArchetypeQuery<T1, T2, T3> query) => this.query = query;
    ...
    where T3 : struct, IComponent
{
    ...
    public Chunks<T1, T2, T3> Current { get; private set; }
    ...
    public bool MoveNext()
    {
        ...
        Current = new Chunks<T1, T2, T3>(chunk1, chunk2, chunk3, entities);
        return true;
        SingleEntity:
        if (pos >= types.last)
        {

...
/// </summary>
/// <remarks>
///     <see cref="Chunk{T}" />'s are typically returned a <see cref="ArchetypeQuery{T1}" />.
///     <see cref="ArchetypeQuery{T1}.Chunks" /> enumerator.<br />
///     <br />
...
///     of
///     <a href="https://github.com/dotnet/runtime/blob/main/docs/coding-guidelines/vectorization-guidelines.md">
///         Vector
///         types
///     </a>
///     <br />
    ...
    where T : struct, IComponent
{
    ...
    /// <summary>
    ///     Return the components as a <see cref="Span{TTo}" /> of type <typeparamref name="TTo" /> - which can be assigned to
    ///     Vector256{TTo}'s.<br />
    ///     The returned <see cref="Span{TTo}" /> contains padding elements on its tail to enable safe conversion to a
    ///     Vector256{TTo}.<br />
    ///     See
    ///     <a href="https://friflo.gitbook.io/friflo.engine.ecs/examples/optimization#query-vectorization---simd">Example.</a>
    ///     .
    /// </summary>
    /// <remarks>
    ///     By adding padding elements the returned <see cref="Span{TTo}" /> can be converted to Vector256's <br />
    ///     without the need of an additional <b>for</b> loop to process the elements at the tail of the <see cref="Span{T}" />
    ///     .<br />
    ///     <br />
...
class ChunkDebugView<T>
    where T : struct, IComponent
{
    [Browse(Never)]
    private readonly Chunk<T> chunk;

    internal ChunkDebugView(Chunk<T> chunk) => this.chunk = chunk;

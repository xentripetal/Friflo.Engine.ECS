...
///     By default, a query does not contain <see cref="Disabled" /> entities. Use <see cref="WithDisabled" /> if needed.
///     <br />
...
public class ArchetypeQuery
{
    ...
    ///     For efficient access to entity <see cref="IComponent" />'s use one of the generic <c>EntityStore.Query()</c>
    ///     methods.
    /// </summary>
    ...
    [Browse(Never)]
    private Archetype[] archetypes; //   8  current list of matching archetypes, can grow
    [Browse(Never)]
    private int[] chunkPositions; //   8  indexes of chunk entities matching a value condition
    [Browse(Never)]
    private EventFilter eventFilter; //   8  used to filter component/tag add/remove events
    [Browse(Never)]
    private EntityList entityList; //   8  provide entities as list to perform structural changes


    // --- blittable types
    [Browse(Never)]
    private int filterVersion; //   4
    [Browse(Never)]
    private int archetypeCount; //   4  current number archetypes 
    [Browse(Never)]
    private int lastArchetypeCount; //   4  number of archetypes the EntityStore had on last check
    [Browse(Never)]
    internal readonly SignatureIndexes signatureIndexes; //  16  ordered struct indices of component types: T1,T2,T3,T4,T5
    [Browse(Never)]
    private readonly ComponentTypes components; //  32
    [Browse(Never)]
    private readonly bool singleArchetype; //   1  if true it returns only the entities a specific archetype
    [Browse(Never)]
    private readonly ComponentType relationQuery; //   8
    ...
    public ArchetypeQuery AllTags(in Tags tags)
    {
        SetHasAllTags(tags);
        return this;
    }
    ...
    /// <param name="tags"> Use <c>Tags.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery AnyTags(in Tags tags)
    {
        SetHasAnyTags(tags);
        return this;
    }
    ...
    public ArchetypeQuery WithDisabled()
    {
        SetWithDisabled();
        return this;
    }
    ...
    /// <param name="tags"> Use <c>Tags.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery WithoutAllTags(in Tags tags)
    {
        SetWithoutAllTags(tags);
        return this;
    }

    /// <summary> Entities having any of the passed <paramref name="tags" /> are excluded from query result. </summary>
    /// <param name="tags"> Use <c>Tags.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery WithoutAnyTags(in Tags tags)
    {
        SetWithoutAnyTags(tags);
        return this;
    }
    ...
    public ArchetypeQuery AllComponents(in ComponentTypes componentTypes)
    {
        SetHasAllComponents(componentTypes);
        return this;
    }

    /// <summary> A query result will contain only entities having any of the the passed <paramref name="componentTypes" />. </summary>
    /// <param name="componentTypes"> Use <c>ComponentTypes.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery AnyComponents(in ComponentTypes componentTypes)
    {
        SetHasAnyComponents(componentTypes);
        return this;
    }

    /// <summary> Entities having all passed <paramref name="componentTypes" /> are excluded from query result. </summary>
    /// <param name="componentTypes"> Use <c>ComponentTypes.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery WithoutAllComponents(in ComponentTypes componentTypes)
    {
        SetWithoutAllComponents(componentTypes);
        return this;
    }

    /// <summary> Entities having any of the passed <paramref name="componentTypes" /> are excluded from query result. </summary>
    /// <param name="componentTypes"> Use <c>ComponentTypes.Get&lt;>()</c> to set the parameter. </param>
    public ArchetypeQuery WithoutAnyComponents(in ComponentTypes componentTypes)
    {
        SetWithoutAnyComponents(componentTypes);
        return this;
    }
    ...
    {
        Filter.HasValue<TComponent, TValue>(value);
        return this;
    }

    /// <inheritdoc cref="QueryFilter.ValueInRange{TComponent,TValue}" />
    public ArchetypeQuery ValueInRange<TComponent, TValue>(TValue min, TValue max)
        where TComponent : struct, IIndexedComponent<TValue> where TValue : IComparable<TValue>
    {
        Filter.ValueInRange<TComponent, TValue>(min, max);
        return this;
    }
    ...
    /// <summary>
    ///     Returns true if a component or tag was added / removed to / from the entity with the passed
    ...
    ///     No structural changes like adding / removing components/tags must not be executed at the same time by another
    ///     thread.
    /// </remarks>
    ...
    internal Archetypes GetArchetypes()
    {
        ...
        for (var n = lastCount; n < newStoreLength; n++)
        {
    ...
    /// <summary>
    ///     Returns true if the passed <paramref name="componentTypes" /> and <paramref name="tags" /> matches the query
    ///     filter.
    /// </summary>
    ...
    internal static ArgumentException ReadOnlyException(Type type) => new ($"Query does not contain Component type: {type.Name}");

    internal string GetQueryChunksString() => signatureIndexes.GetString($"QueryChunks[{ChunkCount}]  Components: ");

    internal string GetQueryJobString() => signatureIndexes.GetString("QueryJob ");

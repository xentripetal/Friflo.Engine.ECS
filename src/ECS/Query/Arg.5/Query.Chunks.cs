    ...
    where T5 : struct, IComponent
{
    ...
    internal QueryChunks(ArchetypeQuery<T1, T2, T3, T4, T5> query) => this.query = query;
    ...
    where T5 : struct, IComponent
{
    ...
    public Chunks<T1, T2, T3, T4, T5> Current { get; private set; }
    ...
    public bool MoveNext()
    {
        ...
        Current = new Chunks<T1, T2, T3, T4, T5>(chunk1, chunk2, chunk3, chunk4, chunk5, entities);
        return true;
        SingleEntity:
        if (pos >= types.last)
        {

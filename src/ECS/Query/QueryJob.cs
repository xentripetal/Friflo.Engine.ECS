...
public abstract class QueryJob
{
    ...
    ///     <see cref="Chunk{T}.AsSpan128{TTo}" />, <see cref="Chunk{T}.AsSpan256{TTo}" /> or
    ///     <see cref="Chunk{T}.AsSpan512{TTo}" />.
    /// </remarks>
    ...
    public ParallelJobRunner JobRunner
    {
        get => GetRunner();
        set => SetRunner(value);
    }
    ...
    /// <summary>
    ///     The minimum number of <see cref="Chunk{T}" /> components per thread required to execute a query
    ///     <see cref="JobExecution.Parallel" />.<br />
    ///     Default: 1000.
    /// </summary>
    /// <remarks>
    ///     Parallel query execution adds an overhead of 1 to 2 micro seconds per query for thread synchronization.<br />
    ///     Execution of a simple computation like <c>health.value++</c> on a single component takes 0.5 to 1 nano seconds.
    ///     <br />
    ///     <br />
    ///     E.g. processing a chunk with 100 components will take 50 to 100 nano seconds.<br />
    ///     So the chunk components are executed <see cref="JobExecution.Sequential" /> to avoid the parallelization overhead.
    ///     <br />
    ///     <br />
    ...
    /// </remarks>
    public int MinParallelChunkLength
    {
        get => minParallel;
        set => SetMinParallel(value);
    }
    ...
    private ParallelJobRunner GetRunner() => jobRunner;
    ...
    internal static InvalidOperationException JobRunnerIsNullException() => new ($"{nameof(QueryJob)} requires a {nameof(JobRunner)}");

    internal bool ExecuteSequential(int taskCount, int chunkLength) => taskCount <= 1 || (chunkLength + taskCount - 1) / taskCount < minParallel;
    ...
    ///     See <see cref="Chunk{T}.AsSpan128{TTo}" />, <see cref="Chunk{T}.AsSpan256{TTo}" /> an
    ///     <see cref="Chunk{T}.AsSpan512{TTo}" />.
    /// </remarks>

...
abstract class ComponentCommands : IComponentStash
{
    [Browse(Never)]
    ...
    internal ComponentCommands(int structIndex) => this.structIndex = structIndex;
...
sealed class ComponentCommands<T> : ComponentCommands, IComponentStash<T>
    where T : struct, IComponent
{
    [Browse(Never)]
    internal ComponentCommand<T>[] componentCommands; //  8
    [Browse(Never)]
    private T stashValue;
    ...
    private static InvalidOperationException EntityNotFound(string command) => new ($"Playback - entity not found. command: {command}");

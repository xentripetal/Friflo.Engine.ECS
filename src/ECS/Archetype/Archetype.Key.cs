...
static class ArchetypeKeyExtensions
{
...
sealed class ArchetypeKeyEqualityComparer : IEqualityComparer<ArchetypeKey>
{
    internal static readonly ArchetypeKeyEqualityComparer Instance = new ();

    public bool Equals(ArchetypeKey left, ArchetypeKey right) => left!.componentTypes.bitSet.Equals(right!.componentTypes.bitSet) &&
                                                                 left!.tags.bitSet.Equals(right!.tags.bitSet);

    public int GetHashCode(ArchetypeKey key) => key.hash;
}

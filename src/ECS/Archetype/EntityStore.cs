...
/// <summary>
///     Store the <see cref="IComponent" />s and <see cref="ITag" /> for the <see cref="Entity" />'s of an
...
public abstract partial class EntityStoreBase
{
    ...
    internal protected abstract void UpdateEntityCompIndex(int id, int compIndex);
    ...
    internal static class Static
    {
        ...
        internal static readonly TypeStore TypeStore = new ();
    ...
    [Browse(Never)]
    public ParallelJobRunner JobRunner { get; init; }
    ...
    [Browse(Never)]
    public int ArchetypeCount => archsCount;
    ...
    /// </summary>
    [Browse(Never)]
    public double ShrinkRatioThreshold
    {
        get => internBase.shrinkRatio;
        set => SetShrinkRatio(value);
    }

    /// <summary> Return the sum of all Archetype capacities. </summary>
    [Browse(Never)]
    public long CapacitySumArchetypes => internBase.archetypesCapacity;
    ...
    [Browse(Never)]
    internal Action<ComponentChanged> ComponentAdded => internBase.componentAdded;
    [Browse(Never)]
    internal Action<ComponentChanged> ComponentRemoved => internBase.componentRemoved;
    [Browse(Never)]
    internal Action<TagsChanged> TagsChanged => internBase.tagsChanged;
    ...
    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.TagsChanged" /> events triggered by:<br />
    ///     <see cref="Entity.AddTag{T}" /> <br /> <see cref="Entity.AddTags" /> <br /> <see cref="Entity.RemoveTag{T}" />
    ///     <br /> <see cref="Entity.RemoveTags" />.
    /// </summary>
    public event Action<TagsChanged> OnTagsChanged
    {
        add => internBase.tagsChanged += value;
        remove => internBase.tagsChanged -= value;
    }

    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.ComponentChanged" /> events triggered by: <br />
    ///     <see cref="Entity.AddComponent{T}()" />.
    /// </summary>
    public event Action<ComponentChanged> OnComponentAdded
    {
        add => internBase.componentAdded += value;
        remove => internBase.componentAdded -= value;
    }

    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.ComponentChanged" /> events triggered by: <br />
    ///     <see cref="Entity.RemoveComponent{T}()" />.
    /// </summary>
    public event Action<ComponentChanged> OnComponentRemoved
    {
        add => internBase.componentRemoved += value;
        remove => internBase.componentRemoved -= value;
    }

    /// <summary>
    ///     Add / remove and event handle for <see cref="ArchetypeCreate" /> events triggered by structural changes or
    ///     entity additions
    /// </summary>
    public event Action<ArchetypeCreate> OnArchetypeCreate
    {
        add => internBase.archetypeAdded += value;
        remove => internBase.archetypeAdded -= value;
    }
    ...
    // --- archetypes
    [Browse(Never)]
    internal Archetype[] archs; //  8   - array of all archetypes. never null
    [Browse(Never)]
    private int archsCount; //  4   - number of archetypes
    [Browse(Never)]
    private readonly HashSet<ArchetypeKey> archSet; //  8   - Set<> to get archetypes by key
    /// <summary>
    ///     The default <see cref="Archetype" /> has no <see cref="Archetype.ComponentTypes" /> and
    ///     <see cref="Archetype.Tags" />.<br />
    ///     Its <see cref="Archetype" />.<see cref="Archetype.archIndex" /> is always 0 (<see cref="Static.DefaultArchIndex" />
    ///     ).
    /// </summary>
    [Browse(Never)]
    internal readonly Archetype defaultArchetype; //  8   - default archetype. has no components & tags
    // --- nodes
    [Browse(Never)]
    internal int entityCount; //  4   - number of all entities
    // --- misc
    [Browse(Never)]
    private readonly ArchetypeKey searchKey; //  8   - key buffer to find archetypes by key
    [Browse(Never)]
    internal readonly int[] singleIds; //  8
    [Browse(Never)]
    internal int singleIndex; //  4
    [Browse(Never)]
    internal bool shrinkArchetypes; //  1
    ...
    internal struct InternBase
    {
        ...
        //
        /// reused query for
        /// <see cref="EntityStoreBase.GetUniqueEntity" />
    ...
    /// </summary>
    internal static NullReferenceException EntityNullException(Entity entity) => new ($"entity is null. id: {entity.Id}");

    internal static ArgumentException EntityArgumentNullException(Entity entity, string param)
        => ExceptionUtils.ArgumentNullException($"entity is null. id: {entity.Id}", param);

    internal static Exception InvalidStoreException(string parameterName)
        => ExceptionUtils.ArgumentException("entity is owned by a different store", parameterName);

    internal static Exception InvalidEntityIdException(int id, string parameterName)
        => ExceptionUtils.ArgumentException($"invalid entity id <= 0. was: {id}", parameterName);

    internal static Exception IdAlreadyInUseException(int id, string parameterName)
        => ExceptionUtils.ArgumentException($"id already in use in EntityStore. id: {id}", parameterName);
    ...
    internal static ArgumentException IdOutOfRangeException(EntityStore store, int id)
        => new ($"id: {id}. expect in [0, current max id: {store.nodes.Length - 1}]");
    ...
    internal static ArgumentException RemoveRelationException(int id, int structIndex)
    {
        ...
        return new ArgumentException(
            $"relation component must be removed with:  entity.{nameof(RelationExtensions.RemoveRelation)}<{type},{keyType}>(key);  id: {id}");
    }
...
{
    /// <summary>
    ///     Safe alternative for unsafe variant using
    ///     <see cref="System.Runtime.InteropServices.MemoryMarshal.CreateReadOnlySpan{T}" />

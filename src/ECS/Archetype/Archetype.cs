...
/// <summary>
///     An <see cref="Archetype" /> store entities with a specific set of <see cref="IComponent" /> and <see cref="ITag" />
///     types.<br />
///     See <a href="https://friflo.gitbook.io/friflo.engine.ecs/examples/general#archetype">Example.</a>
/// </summary>
/// <remarks>
///     E.g. all entities with a <see cref="Position" /> and <see cref="Rotation" /> component are store in the same
///     archetype.<br />
///     In case of removing one of these components or adding a new one from / to an <see cref="Entity" /> the entity is
///     moved to a different archetype.<br />
///     <br />
...
public sealed class Archetype
{
    ...
    [Browse(Never)]
    internal readonly StructHeap[] structHeaps; //  8   - never null. archetype components
    /// Store the entity id for each component.
    [Browse(Never)]
    internal int[] entityIds; //  8   - could use a StructHeap<int> if needed
    [Browse(Never)]
    internal int entityCount; //  4   - number of entities in archetype
    [Browse(Never)]
    private ArchetypeMemory memory; // 16   - count & length used to store components in chunks  
    // --- internal
    [Browse(Never)]
    internal readonly int componentCount; //  4   - number of component types
    [Browse(Never)]
    internal readonly ComponentTypes componentTypes; // 32   - component types of archetype
    [Browse(Never)]
    internal readonly Tags tags; // 32   - tags assigned to archetype
    [Browse(Never)]
    internal readonly ArchetypeKey key; //  8
    /// <remarks>
    ///     Lookups on <see cref="heapMap" />[] does not require a range check. See
    ///     <see cref="EntitySchema.CheckStructIndex" />
    /// </remarks>
    [Browse(Never)]
    internal readonly StructHeap[] heapMap; //  8   - never null. Length always = maxStructIndex. Used for heap lookup
    [Browse(Never)]
    internal readonly EntityStoreBase store; //  8   - containing EntityStoreBase
    [Browse(Never)]
    internal readonly EntityStore entityStore; //  8   - containing EntityStore
    [Browse(Never)]
    internal readonly int archIndex; //  4   - archetype index in EntityStore.archs[]
    [Browse(Never)]
    internal readonly StandardComponents std; // 32   - heap references to std types: Position, Rotation, ...
    [Browse(Never)]
    private ArchetypeQuery query; //  8   - return the entities of this archetype
    [Browse(Never)]
    internal TypeCache componentAdd; // 24
    [Browse(Never)]
    internal TypeCache componentRemove; // 24
    ...
    /// <summary>
    ///     Create an <see cref="Entity" /> with the <see cref="ComponentTypes" /> and <see cref="Tags" /> managed by the
    ///     archetype.
    /// </summary>
    ...
    {
        var localStore = entityStore;
        var compIndexStart = entityCount;
    ...
    private Archetype(in ArchetypeConfig config, StructHeap[] heaps, in Tags tags)
    {
        ...
        this.tags = tags;
        key = new ArchetypeKey(this);
        for (var pos = 0; pos < componentCount; pos++)
        {
            var heap = heaps[pos];
            heap.SetArchetypeDebug(this);
            heapMap[heap.structIndex] = heap;
    ...
    /// <remarks>
    ///     Must be used only on case all <see cref="ComponentTypes" /> are <see cref="ComponentType.IsBlittable" />
    /// </remarks>
    ...
    private static void Resize(Archetype arch, int capacity)
    {
        ...
        var shrinkThreshold = capacity / 4;
...
public static class ArchetypeUtils
{
    ...
    /// <remarks>
    ///     Could be less than 64 if using <see cref="ComponentType{T}.ByteSize" /> for
    ///     <see cref="StructHeap{T}.components" />
    /// </remarks>

...
public static class TreeUtils
{
    ...
    ///     The <see cref="DataEntity.pid" /> and the <see cref="DataEntity.children" /> of the given
    ///     <paramref name="dataEntities" />
    ...
    public static AddDataEntitiesResult AddDataEntitiesToEntity(Entity targetEntity, IReadOnlyList<DataEntity> dataEntities)
    {
        ...
        foreach (var dataEntity in dataEntities)
        {
            ...
            converter.DataEntityToEntity(dataEntity, store, out var error);
            if (error != null)
            {
    ...
    )
    {
        ...
        for (var n = 0; n < children.Count; n++)
        {
            ...
            if (oldToNewPid.TryGetValue(oldPid, out var newPid))
            {
                children[n] = newPid;
                continue;
            }

...
public sealed class EntitySchema
{
    ...
    /// <summary>
    ///     A map to lookup <see cref="ComponentType" />'s and <see cref="ScriptType" />'s by
    ///     <see cref="SchemaType.ComponentKey" />.
    /// </summary>
    ...
    [Browse(Never)]
    private readonly EngineDependant[] engineDependants;
    [Browse(Never)]
    internal readonly int maxStructIndex;
    [Browse(Never)]
    internal readonly int maxIndexedStructIndex; // :)
    [Browse(Never)]
    internal readonly ComponentType[] components;
    [Browse(Never)]
    internal readonly ScriptType[] scripts;
    [Browse(Never)]
    internal readonly TagType[] tags;
    [Browse(Never)]
    internal readonly ComponentType unresolvedType;
    // --- lookup: component / script
    [Browse(Never)]
    internal readonly Dictionary<string, SchemaType> schemaTypeByKey;
    [Browse(Never)]
    internal readonly Dictionary<Type, ScriptType> scriptTypeByType;
    [Browse(Never)]
    private readonly Dictionary<Type, ComponentType> componentTypeByType;
    // --- lookup: tags
    [Browse(Never)]
    internal readonly Dictionary<string, TagType> tagTypeByName;
    [Browse(Never)]
    private readonly Dictionary<Type, TagType> tagTypeByType;
    // --- component type masks
    [Browse(Never)]
    internal readonly ComponentTypes relationTypes;
    [Browse(Never)]
    internal readonly ComponentTypes indexTypes;
    [Browse(Never)]
    internal readonly ComponentTypes linkComponentTypes;
    [Browse(Never)]
    internal readonly ComponentTypes linkRelationTypes;
    ...
    internal EntitySchema(EngineDependant[] dependants, SchemaTypes schemaTypes)
    {
        ...
        var count = componentList.Count + scriptList.Count;
    ...
    /// <remarks>
    ///     Ensures <see cref="StructHeap.structIndex" /> and <see cref="StructInfo{T}.Index" /> is less than
    ///     <see cref="maxStructIndex" /><br />
    ...
    internal int CheckStructIndex(Type structType, int structIndex)
    {
        ...
        {
            var msg = $"number of component types exceed EntityStore.{nameof(maxStructIndex)}: {maxStructIndex}";
            throw new InvalidOperationException(msg);
        }
        return structIndex;
    }

    private string GetString() => $"components: {components.Length - 1}  scripts: {scripts.Length - 1}  entity tags: {tags.Length - 1}";
...
public readonly struct EngineDependant
{
    ...
    [Browse(Never)]
    private readonly SchemaType[] types;

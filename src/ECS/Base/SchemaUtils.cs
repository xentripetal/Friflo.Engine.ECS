...
static class SchemaUtils
{
    ...
    private static EntitySchema RegisterTypes(TypeStore typeStore)
    {
        ...
        for (var n = 0; n < assemblies.Length; n++)
        {
    ...
    internal static ComponentType CreateComponentType<T>(
        TypeStore typeStore,
        int structIndex,
        Type indexType,
        Type indexValueType,
    ...
    private static SymbolColor? ParseColor(string color)
    {
        ...
        if (byte.TryParse(colors[0], out var r) &&
...
readonly struct GenericInstanceType
{
    ...
    internal static void Add(List<GenericInstanceType> list, IList<CustomAttributeTypedArgument> args)
    {
        ...
        {
            case 2:
                list.Add(new GenericInstanceType((string)args[0].Value, new[]
                {
                    (Type)args[1].Value
                }));
                break;
            ...
            case 3:
                list.Add(new GenericInstanceType((string)args[0].Value, new[]
                {
                    (Type)args[1].Value, (Type)args[2].Value
                }));
                break;
            ...
            case 4:
                list.Add(new GenericInstanceType((string)args[0].Value, new[]
                {
                    (Type)args[1].Value, (Type)args[2].Value, (Type)args[3].Value
                }));
                break;
        }
    }

    ...
    where T2 : struct, IComponent
{
    ...
    [Browse(Never)]
    private ArchetypeQuery<T1, T2> query;
    ...
    {
        this.query = (ArchetypeQuery<T1, T2>)query;
    }

    internal override ArchetypeQuery CreateQuery(EntityStore store) => store.Query<T1, T2>(Filter);
}

...
public struct SystemPerf
{
    ...
    ///     Can be 0 in case execution time was below <see cref="Stopwatch.Frequency" /> precision.
    /// </summary>
    ...
    [Browse(Never)]
    internal long sumTicks;
    [Browse(Never)]
    internal long lastMemory;
    [Browse(Never)]
    internal long sumMemory;
    ...
    public float LastAvgMs(int count)
    {
        ...
        var sum = 0L;
        for (var n = updateCount - count; n < updateCount; n++)
        {
            sum += ticks[n % length];
        }
        sum /= count;
...
readonly struct PerfResource
{
    ...
    internal static long GetAllocatedBytes() => Platform.IsUnityRuntime ? 0 : GC.GetAllocatedBytesForCurrentThread();
}

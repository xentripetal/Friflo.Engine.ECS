...
public sealed class CreateEntityBatch
{
    ...
    [Browse(Never)]
    private readonly BatchComponent[] batchComponents; //  8
    [Browse(Never)]
    private readonly ComponentType[] componentTypes; //  8
    [Browse(Never)]
    private readonly EntityStoreBase store; //  8
    [Browse(Never)]
    internal bool autoReturn; //  4
    [Browse(Never)]
    internal bool isReturned; //  4
    [Browse(Never)]
    private Archetype archetype; //  8
    [Browse(Never)]
    private Tags tagsCreate; // 32
    [Browse(Never)]
    private ComponentTypes componentsCreate; // 32
    ...
    /// </summary>
    /// <remarks>
    ///     Subsequent use of a batch returned by <c>Batch(autoReturn: true)</c> throws
    ///     <see cref="ECS.BatchAlreadyReturnedException" />.
    /// </remarks>
    ...
    private static BatchAlreadyReturnedException BatchAlreadyReturnedException() => new ("batch already returned");

    /// <summary>
    ///     Add the given <paramref name="component" /> that will be added to the entity when calling
    ///     <see cref="CreateEntity()" />.
    /// </summary>

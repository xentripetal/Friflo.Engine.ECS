...
public sealed class ExplorerItemTree
{
    ...
    /// <remarks>
    ///     <c>Avalonia.Controls.TreeDataGridItemsSourceView"</c> create items on demand => create <see cref="ExplorerItem" />
    ///     if not present.
    /// </remarks>
    ...
    public bool TryGetItem(int id, out ExplorerItem item) => items.TryGetValue(id, out item);
    ...
    private static Exception InvalidActionException(in ChildEntitiesChanged args) => new InvalidOperationException($"unexpected action: {args.Action}");

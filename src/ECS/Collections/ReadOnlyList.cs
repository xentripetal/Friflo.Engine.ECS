...
public struct ReadOnlyList<T> : IReadOnlyList<T> where T : class
{
    ...
    /// </summary>
    public ReadOnlyListEnumerator<T> GetEnumerator() => new (this);
...
class ReadOnlyListDebugView<T> where T : class
{
    [Browse(Never)]
    private readonly ReadOnlyList<T> readOnlyList;

    internal ReadOnlyListDebugView(ReadOnlyList<T> readOnlyList) => this.readOnlyList = readOnlyList;

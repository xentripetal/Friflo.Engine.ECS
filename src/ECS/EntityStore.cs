...
///                     if <see cref="detached" /> - <see cref="NullReferenceException" /> are thrown by
///                     <see cref="Entity" /> properties and methods.
///                 </item>
///                 <item>
///                     <see cref="TreeMembership" />: <see cref="treeNode" /> / <see cref="floating" /> node (not part of
///                     the <see cref="EntityStore" /> tree graph).<br />
///                     All children of a <see cref="treeNode" /> are <see cref="treeNode" />'s themselves.
///                 </item>
///             </list>
///         </item>
///         <item>
///             Manage a tree graph of entities which starts with the <see cref="StoreRoot" /> entity to build up a scene
///             graph.
///         </item>
...
public sealed partial class EntityStore : EntityStoreBase
{
    ...
    /// </summary>
    public ref readonly EntityNode GetEntityNode(int id) => ref nodes[id];
    ...
    ///     For efficient access to entity <see cref="IComponent" />'s use one of the generic
    ///     <b>
    ...
    /// <summary>
    ///     Record adding/removing of components/tags to/from entities if <see cref="ECS.EventRecorder.Enabled" /> is true.
    ///     <br />
    ///     It is required to filter these events using an <see cref="EventFilter" />.
    /// </summary>
    [Browse(Never)]
    public EventRecorder EventRecorder => GetEventRecorder();
    ...
    /// </summary>
    [Browse(Never)]
    public bool RecycleIds
    {
        get => recycleIds;
        set => SetRecycleIds(value);
    }
    ...
    [Browse(Never)]
    public int NodeMaxId => nodes.Length - 1;
    ...
    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.ChildEntitiesChanged" /> events triggered by:<br />
    ///     <see cref="Entity.AddChild" /> <br /> <see cref="Entity.InsertChild" /> <br /> <see cref="Entity.RemoveChild" />.
    /// </summary>
    public event Action<ChildEntitiesChanged> OnChildEntitiesChanged
    {
        add => extension.childEntitiesChanged += value;
        remove => extension.childEntitiesChanged -= value;
    }

    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.ScriptChanged" /> events triggered by:<br />
    ///     <see cref="Entity.AddScript{T}" />.
    /// </summary>
    public event Action<ScriptChanged> OnScriptAdded
    {
        add => extension.scriptAdded += value;
        remove => extension.scriptAdded -= value;
    }

    /// <summary>
    ///     Add / remove an event handler for <see cref="ECS.ScriptChanged" /> events triggered by:<br />
    ///     <see cref="Entity.RemoveScript{T}" /> .
    /// </summary>
    public event Action<ScriptChanged> OnScriptRemoved
    {
        add => extension.scriptRemoved += value;
        remove => extension.scriptRemoved -= value;
    }
    ...
    {
        add => intern.entitiesChanged += value;
        remove => intern.entitiesChanged -= value;
    }


    /// <summary>
    ///     Add / remove an event handler for <see cref="EntityCreate" /> events triggered by
    ///     <see cref="EntityStore.CreateEntity()" />.
    /// </summary>
    public event Action<EntityCreate> OnEntityCreate
    {
        add => intern.entityCreate += value;
        remove => intern.entityCreate -= value;
    }

    /// <summary>
    ///     Add / remove an event handler for <see cref="EntityDelete" /> events triggered by
    ///     <see cref="Entity.DeleteEntity()" />.
    /// </summary>
    public event Action<EntityDelete> OnEntityDelete
    {
        add => intern.entityDelete += value;
        remove => intern.entityDelete -= value;
    }
    ...
    [Browse(Never)]
    internal EntityNode[] nodes; //   8  - acts also id2pid
    [Browse(Never)]
    private Entity storeRoot; //  16  - origin of the tree graph. null if no origin assigned
    [Browse(Never)]
    private bool recycleIds; //   1

    // --- buffers
    [Browse(Never)]
    private int[] idBuffer; //   8
    [Browse(Never)]
    internal readonly HashSet<int> idBufferSet; //   8
    [Browse(Never)]
    private readonly DataEntity dataBuffer; //   8
    ...
    ///     In case passed <paramref name="id" /> invalid (id >=
    ///     <see cref="Capacity" />).
    /// </exception>

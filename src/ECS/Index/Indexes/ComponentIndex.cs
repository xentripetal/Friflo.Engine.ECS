...
///     A custom component index can be implemented to optimize indexing or component queries for a specific component
///     type.
/// </summary>
public abstract class ComponentIndex
{
    ...
    /// <br />
    /// The component is removed by
    /// <see cref="Entity.DeleteEntity" />
    /// shortly after.
    internal abstract void RemoveEntityFromIndex(int id, Archetype archetype, int compIndex);

    internal NotSupportedException NotSupportedException(string name) => new ($"{name} not supported by {GetType().Name}");

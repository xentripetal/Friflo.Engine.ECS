...
/// <see cref="Archetype" />
/// with a single
/// <see cref="StructHeap{T}" />
/// <br />
class EntityRelationLinks<TRelationComponent> : EntityRelations<TRelationComponent, Entity>
    where TRelationComponent : struct, ILinkRelation
{
    /// Instance created at
    /// <see cref="EntityRelations.GetEntityRelations" />
    ...
    {
        var target = RelationUtils<TComponent, Entity>.GetRelationKey(component);
        var added = true;
    ...
    {
        var position = FindRelationPosition(id, target, out var positions, out var index);
        if (position >= 0)
        {

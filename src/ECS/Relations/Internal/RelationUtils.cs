...
static class RelationUtils
{
    ...
    private static TKey GetRelationComponentKey<TComponent, TKey>(in TComponent component)
        ...
        => component.GetRelationKey();
}

static class RelationUtils<TComponent, TKey>
    where TComponent : struct, IComponent
{
    ...
    static RelationUtils() => GetRelationKey = RelationUtils.CreateGetRelationKey<TComponent, TKey>();
}

delegate TKey GetRelationKey<TComponent, out TKey>(in TComponent component)
    where TComponent : struct, IComponent;

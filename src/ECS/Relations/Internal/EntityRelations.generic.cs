...
/// <br />
class EntityRelations<TRelationComponent, TKey> : EntityRelations
    where TRelationComponent : struct, IRelationComponent<TKey>
{
    /// Single
    /// <see cref="StructHeap" />
    /// stored in the
    /// <see cref="EntityRelations.archetype" />
    /// .
    ...
    {
        var position = FindRelationPosition(id, key, out var positions, out var index);
        if (position >= 0)
        {

...
// ReSharper disable once InconsistentNaming
    public static class Test_Events
    {
        ...
        public static void Test_Events_RemoveHandlersOnDelete()
        {
            ...
            entity1.OnTagsChanged += _ => {
                Fail("unexpected");
            };
            entity1.OnScriptChanged += _ => {
                Fail("unexpected");
            };
            entity1.OnChildEntitiesChanged += _ => {
                Fail("unexpected");
            };
            entity1.AddSignalHandler<MyEvent>(_ => {
                Fail("unexpected");
            });

            var entity2 = store.CreateEntity(2);
            var eventCount = 0;
            entity2.OnComponentChanged += _ => {
                eventCount++;
            };
            entity2.OnTagsChanged += _ => {
                eventCount++;
            };
            entity2.OnScriptChanged += _ => {
                eventCount++;
            };
            entity2.OnChildEntitiesChanged += _ => {
                eventCount++;
            };
            entity2.AddSignalHandler<MyEvent>(_ => {
                eventCount++;
            });
        ...
        public static void Test_Events_add_multiple_signal_handler()
        {
            ...
            Action<Signal<MyEvent1>> handler1 = signal => {
                ...
                {
                    case 0:
                        AreEqual(10, signal.Event.value);
                        break;

                    case 1:
                        AreEqual(30, signal.Event.value);
                        break;
                }
            ...
            Action<Signal<MyEvent2>> handler2A = signal => {
                switch (countEvent2A++)
                {
                    ...
                    case 1:
                        AreEqual(12, signal.Event.value);
                        break;
                }

...
public sealed class StoreCommands : IServiceCommands
{
    ...
    public StoreCommands(EntityStore store) => this.store = store;
    ...
    private static Result<string> Collect(Param<int?> param, MessageContext context)
    {
        ...
        var generation = nullableGeneration ?? 1;
    ...
    async private Task<Result<AddEntitiesResult>> AddEntities(Param<AddEntities> param, MessageContext context)
    {
    ...
    async private Task<Result<GetEntitiesResult>> GetEntities(Param<GetEntities> param, MessageContext context)
    {

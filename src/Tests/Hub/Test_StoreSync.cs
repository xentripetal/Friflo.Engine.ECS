...
namespace Tests.Hub
{
    public static class Test_StoreSync
    {
        ...
        [Test]
        public async static Task Test_StoreSync_load_entities()
        {
            var client = CreateClient();
            var rootNode = new DataEntity
            {
                pid = 10L,
                components = Test_ComponentReader.RootComponents,
            ...
            {
                pid = 11L,
                components = Test_ComponentReader.ChildComponents
            };
        ...
        [Test]
        public async static Task Test_StoreSync_store_entities()
        {
            var client = CreateClient();
            var store = new EntityStore(PidType.UsePidAsId);
            var sync = new StoreSync(store, client);
        ...
        [Test]
        public async static Task Test_StoreSync_SubscribeDatabaseChangesAsync()
        {
            var hub = Prepare_SubscribeDatabaseChanges(out var sync, out var processor);
            var client = new StoreClient(hub);
            var store = sync.Store;

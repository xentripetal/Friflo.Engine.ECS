...
namespace Tests.Hub
{
    public static class Test_StoreCommands
    {
        ...
        [Test]
        public async static Task Test_StoreCommands_Collect()
        {
        ...
        [Test]
        public async static Task Test_StoreCommands_GetEntities()
        {
            var store = new EntityStore(PidType.UsePidAsId);
            store.CreateEntity(11);
            var client = CreateClient(store);
            {
                var get = client.GetEntities(new GetEntities
                {
                    ids = new List<long>
                    {
                        11
        ...
        [Test]
        public async static Task Test_StoreCommands_AddEntities()
        {
            var store = new EntityStore(PidType.UsePidAsId);
            store.CreateEntity(11);
            var client = CreateClient(store);
            {
                ...
                var add = client.AddEntities(new AddEntities
                {
                    targetEntity = 11,
                    entities = entities
                });

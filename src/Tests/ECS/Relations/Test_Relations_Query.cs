...
namespace Tests.ECS.Relations
{
    public static class Test_Relations_Query
    {
        ...
        public static void Test_Relations_query()
        {
            ...
            var count = 0;
            query.ForEachEntity((ref AttackRelation component1, Entity entity) => {
                count++;
            });
            AreEqual(0, count);
            ...
            entity1.AddRelation(new AttackRelation
            {
                target = target10,
                speed = 42
            });

            entity2.AddRelation(new AttackRelation
            {
                target = target10,
                speed = 20
            });
            ...
            entity3.AddRelation(new AttackRelation
            {
                target = target10,
                speed = 10
            });
            ...
            query.ForEachEntity((ref AttackRelation relation, Entity entity) => {
                ...
                {
                    case 0:
                        Mem.AreEqual(42, relation.speed);
                        break;

                    case 1:
                        Mem.AreEqual(20, relation.speed);
                        break;

                    case 2:
                        Mem.AreEqual(21, relation.speed);
                        break;

                    case 3:
                        Mem.AreEqual(10, relation.speed);
                        break;

                    case 4:
                        Mem.AreEqual(11, relation.speed);
                        break;

                    case 5:
                        Mem.AreEqual(12, relation.speed);
                        break;
                }
            });
            ...
            foreach (var entity in query.Entities)
            {
                ...
                {
                    case 0:
                        AreEqual("{ 4 }", relations.Debug());
                        break;

                    case 1:
                        AreEqual("{ 4, 5 }", relations.Debug());
                        break;
        ...
        private static void Relations_add_int_relations_Perf(int componentCount, bool log)
        {
            ...
            sb?.AppendLine("| ------------------:| ------------------:| ------------------:|");
            for (var relationsPerEntity = 1; relationsPerEntity <= 8 * 1024; relationsPerEntity *= 2)
            {
                var store = new EntityStore();
                ...
                while (true)
                {
                    for (var n = 0; n < relationsPerEntity; n++)
                    {
                        createdEntities[entityIndex].AddRelation(new IntRelation
                        {
                            value = n
                        });
        ...
        public static void Test_Relations_ForAllEntityRelations_Perf()
        {
            ...
            foreach (var entity in createdEntities)
            {
                for (var n = 0; n < relationsPerEntity; n++)
                {
                    entity.AddRelation(new IntRelation
                    {
                        value = n
                    });
                }
            ...
            Console.WriteLine(
                $"Test_Relations_ForAllEntityRelations_Perf - entities: {entityCount}  relationsPerEntity: {relationsPerEntity}  duration: {sw.ElapsedMilliseconds} ms");
        }
        ...
        public static void Test_Relations_GetAllEntityRelations_Perf()
        {
            ...
            var sw = new Stopwatch();
            sw.Start();
            (var entities, var relations) = store.GetAllEntityRelations<IntRelation>();
            ...
            AreEqual(entityCount * relationsPerEntity, count);
            Console.WriteLine(
                $"Test_Relations_GetAllEntityRelations_Perf - entities: {entityCount}  relationsPerEntity: {relationsPerEntity}  duration: {sw.ElapsedMilliseconds} ms");
        }

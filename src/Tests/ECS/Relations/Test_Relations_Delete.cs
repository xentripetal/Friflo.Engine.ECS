...
namespace Tests.ECS.Relations
{
    public static class Test_Relations_Delete
    {
        ...
        public static void Test_Relations_Delete_int_relations()
        {
            ...
            entity2.AddRelation(new IntRelation
            {
                value = 30
            });
            ...
            store.ForAllEntityRelations((ref IntRelation relation, Entity entity) => {
                switch (count++)
                {
                    case 0:
                        ...
                        AreEqual(10, relation.value);
                        break;

                    case 1:
                        AreEqual(1, entity.Id);
                        AreEqual(20, relation.value);
                        break;

                    case 2:
                        AreEqual(2, entity.Id);
                        AreEqual(30, relation.value);
                        break;
                }
            });
            AreEqual(3, count);
        ...
        public static void Test_Relations_Delete_Entity_relations()
        {
            ...
            store.ForAllEntityRelations((ref AttackRelation relation, Entity entity) => {
                switch (count++)
                {
                    case 0:
                        AreEqual(1, entity.Id);
                        AreEqual(10, relation.target.Id);
                        break;

                    case 1:
                        AreEqual(1, entity.Id);
                        AreEqual(11, relation.target.Id);
                        break;

                    case 2:
                        AreEqual(2, entity.Id);
                        AreEqual(12, relation.target.Id);
                        break;
                }
            });
            AreEqual(3, count);
            ...
            // --- version: get all entity relations in O(1)
            (var entities, var relations) = store.GetAllEntityRelations<AttackRelation>();
            AreEqual("{ 1, 1, 2 }", entities.Debug());

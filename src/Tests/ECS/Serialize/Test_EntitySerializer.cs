...
namespace Tests.ECS.Serialize
{
    ...
    ""components"": {
        ...
        async private static Task Serializer_write_store()
        {
        ...
        async private static Task Serializer_read_into_store_async()
        {
            var store = new EntityStore(PidType.UsePidAsId);
            var serializer = new EntitySerializer();
        ...
        public static void Test_Serializer_write_store_Perf()
        {
            ...
            var count = 10; // 1_000_000 ~ #PC: 1.227 ms
        ...
        {
            var entityCount = 100; // 1_000_000 ~ #PC: 2367 ms
            var stream = new MemoryStream();
        ...
        async private static Task Serializer_ReadAsync_MemoryStream()
        {
            var stream = new MemoryStream();
            var store = new EntityStore(PidType.UsePidAsId);
            var serializer = new EntitySerializer();

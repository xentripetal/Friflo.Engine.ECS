...
// ReSharper disable once InconsistentNaming
    ...
    {
        /// <summary>
        ///     Need to create <see cref="JsonValue" /> instances. They can be modified in
        ///     <see cref="Friflo.Json.Fliox.JsonValue.Copy" />
        /// </summary>
        internal static JsonValue RootComponents =>
            new ("{ \"pos\": { \"x\": 1, \"y\": 1, \"z\": 1 }, \"scl3\": { \"x\": 2, \"y\": 2, \"z\": 2 } }");

        internal static JsonValue ChildComponents => new ("{ \"pos\": { \"x\": 3, \"y\": 3, \"z\": 3 }, \"scl3\": { \"x\": 4, \"y\": 4, \"z\": 4 } }");

        private static JsonValue Script => new ("{ \"script1\": { \"val1\": 2 } }");

        private static JsonValue Scripts => new (
        ...
        public static void Test_ComponentReader_read_children_events()
        {
            ...
            var events = Events.SetHandlerSeq(store, (args, seq) => {
                ...
                switch (seq)
                {
                    ...
                    case 0:
                        AreEqual("entity: 1 - event > Add Child[0] = 2", str);
                        return;

                    case 1:
                        AreEqual("entity: 1 - event > Add Child[1] = 3", str);
                        return;

                    case 2:
                        AreEqual("entity: 1 - event > Add Child[2] = 4", str);
                        return;

                    case 3:
                        AreEqual("entity: 1 - event > Add Child[3] = 5", str);
                        return;

                    // --- changed children: [6, 4, 2, 5]
                    case 4:
                        AreEqual("entity: 1 - event > Remove Child[1] = 3", str);
                        return;

                    case 5:
                        AreEqual("entity: 1 - event > Add Child[0] = 6", str);
                        return;

                    case 6:
                        AreEqual("entity: 1 - event > Remove Child[2] = 4", str);
                        return;

                    case 7:
                        AreEqual("entity: 1 - event > Remove Child[1] = 2", str);
                        return;

                    case 8:
                        AreEqual("entity: 1 - event > Add Child[1] = 4", str);
                        return;

                    case 9:
                        AreEqual("entity: 1 - event > Add Child[2] = 2", str);
                        return;

                    default:
                        Fail($"unexpected seq: {seq}");
                        return;
                }
            });
            ComponentReader_read_children(store);
        ...
        private static void ComponentReader_read_children(EntityStore store)
        {
            ...
            for (var n = 2; n <= 6; n++)
            {
                converter.DataEntityToEntity(new DataEntity
                {
                    pid = n
                }, store, out _);
            }
        ...
        public static void Test_ComponentReader_read_EntityName()
        {
            ...
            {
                pid = 10,
                components = new JsonValue("{\"name\":{\"value\":\"test\"}}")
            };
            var entity = converter.DataEntityToEntity(node, store, out var error);
        ...
        public static void Test_ComponentReader_read_invalid_components()
        {
            ...
            {
                pid = 10,
                components = new JsonValue("invalid")
            };
        ...
        public static void Test_ComponentReader_read_component_error()
        {
            ...
            {
                pid = 10,
                components = new JsonValue("{\"pos\":{\"x\":[]}}")
            };
            var entity = converter.DataEntityToEntity(node, store, out var error);

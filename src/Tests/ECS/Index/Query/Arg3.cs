...
namespace Tests.ECS.Index.Query
{
    public static partial class Test_Index_Query
    {
        private static void QueryArg3(IndexContext cx)
        {
            ...
            var query3 = store.Query<Position, Rotation, MyComponent1>().HasValue<IndexedName, string>("find-me").HasValue<IndexedInt, int>(123);
            ...
            {
                var count = 0;
                foreach ((var positions, _, _, var entities) in query5.Chunks)
                {

...
namespace Tests.ECS.Index
{
    public static class Test_Index_Range
    {
        ...
        public static void Test_Index_Range_Query_ValueInRange()
        {
            ...
            entity1.AddComponent(new IndexedIntRange
            {
                value = 100
            });
            AreEqual("{ 100 }", values.Debug());
            entity2.AddComponent(new IndexedIntRange
            {
                value = 200
            });
            AreEqual("{ 100, 200 }", values.Debug());
            entity3.AddComponent(new IndexedIntRange
            {
                value = 300
            });
            AreEqual("{ 100, 200, 300 }", values.Debug());

            var result = store.GetEntitiesWithComponentValue<IndexedIntRange, int>(100);
            AreEqual(1, result.Count);
            AreEqual("{ 1 }", result.Ids.Debug());
            result = store.GetEntitiesWithComponentValue<IndexedIntRange, int>(42);
            AreEqual(0, result.Count);
            AreEqual("{ }", result.Ids.Debug());
            {
                var count = 0;
            ...
            AreEqual(0, query0.Entities.Count);
            AreEqual("{ }", query0.Entities.Debug());
            AreEqual(1, query1.Entities.Count);
            AreEqual("{ 1 }", query1.Entities.Debug());
            AreEqual(2, query2.Entities.Count);
            AreEqual("{ 1, 2 }", query2.Entities.Debug());
            AreEqual(3, query3.Entities.Count);
            AreEqual("{ 1, 2, 3 }", query3.Entities.Debug());
            AreEqual(0, query4.Entities.Count);
            AreEqual("{ }", query4.Entities.Debug());
        ...
        public static void Test_Index_Range_Update_Remove()
        {
            ...
            AreEqual("{ 100, 200 }", values.Debug());
            entity4.AddComponent(new IndexedIntRange
            {
                value = 300
            });
            AreEqual("{ 100, 200, 300 }", values.Debug());
            ...
            AreEqual("{ 100, 200, 300 }", values.Debug()); // cover add same component value again
            ...
            AreEqual(4, query1.Entities.Count);
            AreEqual("{ 1, 2, 3, 4 }", query1.Entities.Debug());
            AreEqual(3, query2.Entities.Count);
            AreEqual("{ 2, 3, 4 }", query2.Entities.Debug());

            entity1.AddComponent(new IndexedIntRange
            {
                value = 400
            });
            AreEqual("{ 400, 200, 300 }", values.Debug());
            AreEqual(3, query1.Entities.Count);
            AreEqual("{ 2, 3, 4 }", query1.Entities.Debug());
            AreEqual(4, query2.Entities.Count);
            AreEqual("{ 2, 3, 4, 1 }", query2.Entities.Debug());

            entity2.RemoveComponent<IndexedIntRange>();
            AreEqual("{ 400, 200, 300 }", values.Debug());
            AreEqual(2, query1.Entities.Count);
            AreEqual("{ 3, 4 }", query1.Entities.Debug());
            AreEqual(3, query2.Entities.Count);
            AreEqual("{ 3, 4, 1 }", query2.Entities.Debug());
        }
        ...
        public static void Test_Index_Range_Allocation()
        {
            ...
            for (var n = 0; n < count; n++)
            {
                ...
                entities[n].AddComponent(new IndexedStringRange
                {
                    value = strings[n]
                });
            }
        ...
        public static void Test_Index_Allocation()
        {
            ...
            for (var n = 0; n < count; n++)
            {
                entities[n].AddComponent(new IndexedIntRange
                {
                    value = count + n
                });
            }
